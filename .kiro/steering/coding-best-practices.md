...
> **Adoption status (2026-09):** This is the target standard. A few items are not yet wired into the codebase and are tracked as a dedicated adoption effort — do **not** assume they exist until then: **Flyway migrations + `ddl-auto=validate`** (currently `ddl-auto=update`, no `db/migration`), **`BusinessRuleException`** (not yet created — existing services use `IllegalArgumentException`/`IllegalStateException`), **`@WebMvcTest` controller tests / Testcontainers / ArchUnit / `spring-security-test`** (not yet added), and **DTO-only controller responses** (`AuthController` still returns a `Map`). When you touch these areas, follow the target rule for *new* code where practical and flag any gap, but a full migration is its own task — do not silently retrofit the whole project mid-feature.
