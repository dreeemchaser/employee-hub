...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Void>> handleOptimisticLock(OptimisticLockingFailureException ex) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(ApiResponse.error("This record was modified by someone else. Please reload and try again."));
    }
    ...
    // Thrown by @Validated on @RequestParam/@PathVariable method parameters (distinct from
    // MethodArgumentNotValidException, which only covers @Valid @RequestBody). Without this handler
    // a constraint failure here would fall through to the generic 500 handler below.
    ...
    public ResponseEntity<ApiResponse<Void>> handleExpiredJwt(ExpiredJwtException ex) {
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(ApiResponse.error("Token has expired"));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleMaxUploadSize(MaxUploadSizeExceededException ex) {
        return ResponseEntity.status(HttpStatus.PAYLOAD_TOO_LARGE).body(ApiResponse.error("File size exceeds the allowed limit"));
    }
